...
def create_lhs(parameter_file       # Parameter file from CONNECT
           ):
    ...
    if not os.path.isfile(CONNECT_PATH+f'/data/lhs_samples/sample_models_{param.jobname}_{param.N}.txt'):
        lhs = LatinHypercubeSampler(param)
        lhs.run(CONNECT_PATH)


def create_output_folders(parameter_file,    # Parameter file from CONNECT
    ...
    if iter_num == None:
        os.system(f"rm -rf {CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}'}")
        os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}')
        os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}/model_params_data')
        for output in param.output_Cl:
            os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}/Cl_{output}_data')
        for output in param.output_Pk:
            os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}/Pk_{output}_data')
        for output in param.output_bg:
            os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}/{output}_data')
        for output in param.output_th:
            os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}/{output}_data')
        if len(param.output_derived) > 0:
            os.mkdir(CONNECT_PATH+f'/data/{param.jobname}/N-{param.N_initial}/derived_data')

    else:
        os.system(f"rm -rf {CONNECT_PATH+f'/data/{param.jobname}/number_{iter_num}/model_params_data'}")
...
def Gelman_Rubin_log(parameter_file,  # Parameter file from CONNECT
    ...
    if status == 'initial':
        ...
        if any('Removed everything: chain not converged' in s for s in output):
            ...
            with open(CONNECT_PATH+f'/data/{param.jobname}/Gelman-Rubin.txt', 'a') as f:
                f.write(Rm1_line)
            os.system(f'echo "Iteration {status} and {status-1} has the following R-1 values"')
...
def filter_chains(parameter_file,   # Parameter file from CONNECT
...
def join_data_files(parameter_file  # Parameter file from CONNECT
                ):
...
def combine_iterations_data(parameter_file,    # Parameter file from CONNECT  
                            iter_num           # Current iteration number
                        ):
