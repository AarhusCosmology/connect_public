...
class CreateSingleDataFile():
    def __init__(self, param, CONNECT_PATH):
        ...
        if len(self.param.output_Cl) > 0:
            ...
            for filename in sorted(os.listdir(self.path + f'Cl_{Cl_out_1}_data')):
                if filename.endswith('.txt'):
                    with open(self.path + f'Cl_{Cl_out_1}_data/'+filename,'r') as f:
                        try:
            ...
            files = sorted(os.listdir(self.path + f'Cl_{Cl_out_1}_data'))
            i = 0
            while not self.ell_common:
                if files[i].endswith('.txt'):
                    with open(self.path + f'Cl_{Cl_out_1}_data/'+files[i],'r') as f:
                        f_list = list(f)
    ...
    def join(self):
        ...
        for output in self.param.output_Cl:
            ...
            with open(self.path + f'Cl_{output}.txt','w') as f:
                for filename in sorted(os.listdir(self.path + f'Cl_{output}_data')):
                    if filename.endswith('.txt'):
                        with open(self.path + f'Cl_{output}_data/'+filename,'r') as g:
                            g_list = list(g)
                            if Cl_header:
        ...
        for output in self.param.output_Pk:
            with open(self.path + f'Pk_{output}.txt','w') as f:
                for filename in sorted(os.listdir(self.path + f'Pk_{output}_data')):
                    if filename.endswith('.txt'):
                        with open(self.path + f'Pk_{output}_data/'+filename,'r') as g:
                            for line in g:
                                f.write(line)


        for output in self.param.output_bg + self.param.output_th:
            with open(self.path + f'{output}.txt','w') as f:
                for filename in sorted(os.listdir(self.path + f'{output}_data')):
                    if filename.endswith('.txt'):
                        with open(self.path + f'{output}_data/'+filename,'r') as g:
                            for line in g:
                                f.write(line)
        ...
        with open(self.path + 'model_params.txt','w') as f:
            for filename in sorted(os.listdir(self.path + 'model_params_data')):
                if filename.endswith('.txt'):
                    with open(self.path + 'model_params_data/'+filename,'r') as g:
                        for line in g:
                            if line[0] == '#' and i == 0:
                                f.write(line)
                                i = 1
