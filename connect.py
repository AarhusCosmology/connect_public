...
create_parser.add_argument("-s", "--sampling", help="Method of sampling", choices=["lhs","iterative"])
...
if keyword == 'create':
    if param.sampling == 'iterative':
        ...
        os.system(f"sbatch --ntasks={param.N_cpu+1} --output={job_out} source/iterative_sampling_method.js {CONNECT_PATH} {param_file} {passphrase} cpu")
    else:
...
srun -n {param.N_cpu + 1} python {CONNECT_PATH}/source/calc_models_mpi.py {f'data/{param.jobname}/{Nfolder}'}/log_connect.param {CONNECT_PATH} lhs
...
def join_output_files():
    try:
        ...
        if param.montepython_data and not os.path.isfile(CONNECT_PATH + f'/data/{param.jobname}/number_{i}/model_params.txt'):
            from source.join_output import CreateSingleDataFile
            CSDF = CreateSingleDataFile(param, CONNECT_PATH)
            CSDF.join()
        elif param.montepython_data and not os.path.isfile(CONNECT_PATH + f'/data/{param.jobname}/montepython_data/model_params.txt') and not os.path.isdir(CONNECT_PATH + f'/data/{param.jobname}/number_{i}'):
            from source.convert_montepython_data import CreateSingleDataFile
            CSDF = CreateSingleDataFile(param, CONNECT_PATH)
            CSDF.join()
    except:
